...
int32_t SchedulerContext::handshake_all_cores(Runtime *runtime) {
    ...
    for (int32_t i = 0; i < cores_total_num_; i++) {
        ...
        if (type == CoreType::AIC) {
            ...
            DEV_INFO("Core %d: AIC, die%u, physical_id=%u, reg_addr=0x%lx", i, die_idx, physical_core_id, reg_addr);
        } else {
            aiv_worker_ids_[aiv_count_++] = i;
            DEV_INFO("Core %d: AIV, die%u, physical_id=%u, reg_addr=0x%lx", i, die_idx, physical_core_id, reg_addr);
        }
    }
