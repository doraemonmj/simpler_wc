...
bool platform_aicpu_affinity_gate(int32_t logical_count, int32_t total_launched) {
    ...
    LOG_INFO("AICPU affinity gate: thread idx=%d sched_getcpu=%d", idx, cpu);
    ...
    if (s_gate_init.compare_exchange_strong(expected, 1, std::memory_order_acq_rel, std::memory_order_acquire)) {
        ...
        LOG_INFO(
            "AICPU affinity gate: allowed_cnt=%d logical=%d", allowed_cnt, logical_count
        );

        if (allowed_cnt == logical_count) {
            // Expected topology: allowlist threads survive
        } else {
            // Unexpected topology: fall back to first logical_count threads
            LOG_WARN(
                "AICPU affinity gate: unexpected topology (allowed=%d expected=%d), "
                "falling back to index-based cutoff",
                allowed_cnt, logical_count
            );
            for (int32_t i = 0; i < total_launched; ++i) {
                s_thread_survive[i] = false;
            }
    ...
    if (!survive) {
        LOG_INFO("AICPU affinity gate: thread idx=%d cpu=%d DROPPED", idx, cpu);
    } else {
        LOG_INFO("AICPU affinity gate: thread idx=%d cpu=%d ACTIVE", idx, cpu);
    }
