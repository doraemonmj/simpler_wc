...
def _svg_timeline(records: list[dict], resource: str, size: int | None, unit: str, fmt: str, divisor: int) -> str:
    ...
    parts.append(f'<text x="{_PAD_L - 6}" y="{sy(0):.1f}" class="ylab" text-anchor="end">0</text>')
    parts.append(f'<text x="{_PAD_L}" y="{_SVG_H - 8}" class="alab">record stream (begin / task / end)</text>')
    parts.append(
...
def _ring_section(
    ...
    charts = [_svg_timeline(ordered, resource, size, unit, fmt, divisor)]
    ...
    return (
        f'<section class="ring"><h3>ring {ring} — {_esc(resource)} '
        f"({len(ordered)} records, {len(pairs)} scope pairs)</h3>{''.join(charts)}</section>"
    )
...
<p>The first chart in each ring is the <span class="name">task timeline</span>:
live occupancy sampled at every record in stream order, so per-task growth is
