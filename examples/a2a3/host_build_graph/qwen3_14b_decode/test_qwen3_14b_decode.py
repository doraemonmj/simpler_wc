...
# -----------------------------------------------------------------------------------------------------------
"""Qwen3-14B 40-layer decode coverage for host-built Graph Execution.

Layer 0 records and executes one decoder-layer task DAG. Layers 1 through 39
reuse the cached Definition and submit one outer Graph task apiece with their
own weight, KV-cache, hidden-state, and scratch bindings.
"""
...
QWEN_KERNELS_DIR = REPO_ROOT / "examples/a2a3/tensormap_and_ringbuffer/qwen3_14b_decode"


def _load_qwen_case():
    module_name = "_qwen3_14b_host_build_graph_base"
    spec = importlib.util.spec_from_file_location(module_name, QWEN_KERNELS_DIR / "test_qwen3_14b_decode.py")
...
def _callable():
    ...
    for incore in callable_cfg["incores"]:
        incore["source"] = str(QWEN_KERNELS_DIR / incore["source"])
    return callable_cfg


@scene_test(level=2, runtime="host_build_graph")
class TestQwen314BDecodeHostBuildGraph(SceneTestCase):
    """Qwen3-14B decode with one recorded decoder-layer Graph reused 40 times."""
